"""Geodesic attention extracted behind the independent engine boundary."""
...
class GeodesicAttention:
    ...
    def __call__(
        ...
        if (
            not torch.isfinite(queries).all()
        ...
        if self.translator is not None:
            translator_output = self.translator(
                queries, keys, distances, previous_queries
            )
