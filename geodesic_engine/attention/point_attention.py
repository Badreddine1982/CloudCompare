"""Point attention extracted behind the independent engine boundary."""
...
class PointAttentionAssistant:
    ...
    def __call__(
        ...
        if neighborhoods.ndim != 4:
            raise ValueError(
                "neighborhoods must have shape (batch, points, neighbors, dim)"
            )
        if (
            local_states.shape[:2] != neighborhoods.shape[:2]
            or local_states.shape[-1] != neighborhoods.shape[-1]
        ):
            raise ValueError(
                "local_states and neighborhoods must agree on batch, point, and feature dimensions"
            )
        if (
            not torch.isfinite(local_states).all()
            or not torch.isfinite(neighborhoods).all()
        ):
            raise ValueError("PointAttentionAssistant rejects NaN/Inf inputs")

        dim = local_states.shape[-1]
        scores = (
            (local_states.unsqueeze(-2) * neighborhoods).sum(dim=-1)
            / math.sqrt(dim)
        )
        weights = torch.softmax(scores, dim=-1)
        if not torch.isfinite(weights).all():
            raise ValueError(
                "PointAttentionAssistant produced non-finite attention weights"
            )
        points = (weights.unsqueeze(-1) * neighborhoods).sum(dim=-2)
        return PointAttentionOutput(
            points=points,
