...
class GeodesicTranslator:
    ...
    def __init__(self, gamma_max: float = 0.1, pressure_thresholds: tuple[float, float, float, float] = (0.25, 0.5, 0.8, 1.2), controller: Any | None = None) -> None:
        if torch is None:
    ...
    def measure(self, queries: "torch.Tensor", keys: "torch.Tensor", geodesic_distances: "torch.Tensor", previous_queries: "torch.Tensor | None" = None) -> TranslatorMeasurements:
        ...
        state_velocity = 0.0 if previous_queries is None else float((queries - previous_queries).norm(dim=-1).mean().detach().cpu())
    ...
    def _raw_state(self, queries: "torch.Tensor", keys: "torch.Tensor", geodesic_distances: "torch.Tensor", previous_queries: "torch.Tensor | None", measurements: TranslatorMeasurements, device: "torch.device | None", dtype: "torch.dtype | None") -> "torch.Tensor":
        ...
        velocity = torch.zeros((), device=geodesic_distances.device, dtype=geodesic_distances.dtype) if previous_queries is None else (queries - previous_queries).norm(dim=-1).mean()
        key_count = int(geodesic_distances.shape[-1])
        if key_count <= 1:
    ...
    def correction(self, scores_shape: tuple[int, ...], measurements: TranslatorMeasurements, device: "torch.device | None" = None, dtype: "torch.dtype | None" = None, raw: "torch.Tensor | None" = None) -> "torch.Tensor":
        resolved_dtype = dtype or torch.float32
        if raw is None:
            raw = torch.as_tensor(measurements.pressure + 0.5 * measurements.geodesic_error + 0.25 * measurements.mirror_error, device=device, dtype=resolved_dtype)
