...
bool CommandExtractCCs::process(ccCommandLineInterface& cmd)
{
	...
	bool success = true; // any error while processing a cloud makes the whole command fail
	...
	try
	{
		...
		for (CLCloudDesc& desc : inputClouds)
		{
			...
			if (sfIdx < 0)
			{
				success = cmd.error(QObject::tr("Couldn't allocate a new scalar field for computing CC labels! Try to free some memory ..."));
				continue;
			}
			...
			if (componentCount == 0)
			{
				success = cmd.error(QObject::tr("No component found!"));
				continue;
			}

			desc.pc->getCurrentInScalarField()->computeMinAndMax();
			CCCoreLib::ReferenceCloudContainer components;
			bool                               extractionSuccess = CCCoreLib::AutoSegmentationTools::extractConnectedComponents(desc.pc, components);
			desc.pc->deleteScalarField(sfIdx);
			sfIdx = -1;

			if (!extractionSuccess)
			{
				success = cmd.error(QObject::tr("An error occurred (failed to finish the extraction of cloud '%1')").arg(desc.pc->getName()));
				continue;
			}
			...
			for (size_t j = 0; j < components.size(); ++j)
			{
				...
				if (compIndexes->size() >= minPointCount)
				{
					...
					{
						success = cmd.error(QObject::tr("Failed to create component #%1! (not enough memory)").arg(j + 1));
					}
				}
			...
			if (cmd.clouds().empty())
			{
				success = cmd.error(QObject::tr("No component was created! Check the minimum size..."));
			}
			else
			{
...
bool CommandCPS::process(ccCommandLineInterface& cmd)
{
	...
	if (result < 0)
	{
		return cmd.error(QObject::tr("Failed to compute the closest point set!"));
	}

	{
		...
		if (!newCloud)
		{
			return cmd.error(QObject::tr("Not enough memory to extract the closest point set!"));
		}
	...
	for (CLCloudDesc& desc : cmd.clouds())
	{
		...
		if (outSF)
		{
			...
			if (!theOctree)
			{
				...
				{
					delete distrib;
					return cmd.error(QObject::tr("Couldn't compute octree for cloud '%1'!").arg(desc.pc->getName()));
				}
			}
